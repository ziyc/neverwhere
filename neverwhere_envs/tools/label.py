...
from vuer.schemas import DefaultScene, TriMesh, Cylinder, Movable, InputBox, AutoScroll, div, Sphere, Plane
...
class Args(ParamsProto):
    ...
    # xml template
    go1_template_path = "neverwhere_envs/tools/templates/mesh_go1.xml"
    go2_template_path = "neverwhere_envs/tools/templates/mesh_go2.xml"
    # mesh params
    bounding_box = [-2, -3, -3, 7, 3, 3]
    friction = 1.25
...
def main(**deps):
    ...
    async def s(event: ClientEvent, session: VuerSession):
        ...
        if key == "m":
            ...
            session.remove @ "trimesh"
            session.remove @ "mesh"
            session.remove @ "marker_0"
            session.remove @ "marker_1"
            session.remove @ "sphere_0"
            session.remove @ "sphere_1"
        ...
        if key == "s":
            ...
            # Save for both Go1 and Go2 templates
            for template_path in [Args.go1_template_path, Args.go2_template_path]:
                # Modify save path to include robot type
                robot_type = "go1" if template_path == Args.go1_template_path else "go2"
                scene_stem = str(Path(Args.scene_name).stem)
                current_save_path = save_path.replace(f"{scene_stem}.xml",  f"{Args.prefix}{robot_type}-{scene_stem}.xml")
                ...
                tree = ET.parse(current_save_path)
                root = tree.getroot()
                ...
                tree.write(current_save_path, encoding="unicode", xml_declaration=True)

                if Args.auto_add:
                    task_folder = Path(__file__).parent.parent.parent / "neverwhere" / "tasks"
                    task_filename = f"{Args.prefix}{robot_type}-{scene_stem}"
                    tree.write(str(task_folder / f"{task_filename}.xml"))

                    print(f"Added to dog park task list..{task_folder / f'{task_filename}.xml'}")
    ...
    async def main(session):
        children = [
            # NOTE(ziyu): add if need reference
            # Plane(
            #     color="red",
            #     key="ground",
            #     scale=100
