...
def add_waypoint_bodies(root):
    ...
    if worldbody is not None:
        for i in range(len(SaveArgs.waypoints)):
            ...
            rotation = " ".join(map(str, SaveArgs.waypoints[i]["rotation"]))
            body_elem = ET.Element("body", {"name": f"waypoint-{i}", "mocap": "true", "pos": position, "euler": rotation})
...
def main(**deps):
    ...
    async def s(event: ClientEvent, session: VuerSession):
        ...
        if key == "n":
            ...
            rotation = [0, 0, 0] if waypoint_num == 0 else \
                       SaveArgs.waypoints[waypoint_num-1]["rotation"]
            ...
            scene_stem = Path(Args.scene_name).stem
            
            # Save for both go1 and go2
            for robot_type in ["go1", "go2"]:
                task_filename = f"{Args.prefix}{robot_type}-{scene_stem}"
                save_path = f"neverwhere/tasks/{task_filename}.xml"
                ...
                # modify collision_mesh's pos and euler according to args.position and rotation
                if worldbody is not None:
                    scene_group = worldbody.find(".//body[@name='scene-group-2']")
                    if scene_group is not None:
                        mesh_body = scene_group.find(".//body[@name='collision_mesh']")
